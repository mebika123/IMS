...
const EditCustomer = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        console.log(form);
        try {
            const res = await axios.put(`/customer/update/${id}`, form);
            if (res.data.status) {
    ...
    return (
        <div className="shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] bg-white border border-[#E5E7EB] rounded-sm p-3 w-3/4 mx-auto mb-4">
            <h3 className="text-bold text-xl">Edit Customer</h3>
            <div className="mt-5">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className='text-center'>
                        <input type="submit" value="Edit" className='w-32 rounded-lg bg-darkgreen text-white p-2 mt-1 cursor-pointer' />
                    </div>
                </form>
            </div>
        </div>
